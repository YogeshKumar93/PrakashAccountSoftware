...
export const LevinDmt = () => {
  ...
  const [showOtpModal, setShowOtpModal] = useState(false); // Add OTP modal state
  const [otpData, setOtpData] = useState(null); // Add OTP data state
    ...
    try {
      ...
      if (response) {
        ...
        if (
          ...
          setSenderData(null);
        } else if (message === "OTP has successfully sent on your registered mobile number") {
  ...
  const handleOtpSuccess = (token) => {
    ...
    showToast("OTP verified successfully! Fetching sender details...", "success");
  };
  ...
  return (
    <Box sx={{ maxWidth: 800, margin: "auto", p: 2 }}>
      ...
      {/* Show registration component when sender not found */}
      ...
      {/* Show OTP modal when OTP response received */}
